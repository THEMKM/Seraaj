...
export default function OpportunityDetail() {
  const { id } = useParams();
  const { data } = useQuery(['opp', id], async () => {
  ...
  return (
    <div className="p-4 md:flex md:gap-8">
      <div className="md:flex-1">
        ...
        <button className="rounded-2xl bg-brand px-4 py-2 text-white">Apply Now</button>
      </div>
      <aside className="mt-8 md:mt-0 md:w-1/3">
        <OpportunityCard title={data.title} orgName={data.orgName} matchScore={data.matchScore} />
      </aside>
    </div>
  );
}
